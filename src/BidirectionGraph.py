...
def load_dumped_quotes(directory):
    ...
    for file in glob.glob("*.json"):
        ...
        men_df_grouped = men_df.groupby(['mentioner','mentioned']).agg({'sentiment':'mean'}).reset_index()
        men_df_grouped['topic_index'] = men_df.groupby(['mentioner','mentioned'])['topic_index'].agg(lambda x: pd.Series.mode(x).iat[0]).reset_index()['topic_index']
        men_df_weight = men_df.groupby(['mentioner','mentioned']).size().reset_index(name='weight')
        men_df_grouped['weight'] = men_df_weight['weight']
        quotes_df = pd.concat([quotes_df, men_df_grouped], axis=0)
    quotes_df_final = quotes_df.groupby(['mentioner','mentioned']).agg({'weight':'sum'}).reset_index()
    quotes_df_final['sentiment']=quotes_df.groupby(['mentioner','mentioned']).apply(lambda x: pd.Series([np.average(x['sentiment'], weights=x['weight'])])).reset_index()[0]
    quotes_df_final['topic_index']=quotes_df.groupby(['mentioner','mentioned'])['topic_index'].agg(lambda x: pd.Series.mode(x).iat[0]).reset_index()['topic_index']
    quotes_df_final['topic_index'] = quotes_df_final['topic_index'].apply(topic_reclustering)
    ...
    quotes_df_all = [quotes_df_0,quotes_df_1,quotes_df_2,quotes_df_3,quotes_df_4]
    return(quotes_df_all)
def construct_network_by_topic(quotes_df_all)
        ...
        for i,row in men_df_grouped.iterrows():
            if not G.has_node(row.mentioner):
                if (row.mentioner.lower() in catalogue_df.name_low.values):
                    name = catalogue_df.loc[catalogue_df['name_low'] == row.mentioner.lower()].name.values[0]
                    if not G.has_node(name):
                        G.add_node(name)
                        G.nodes[name]['party']= catalogue_df.loc[catalogue_df['name_low'] == row.mentioner.lower()].parties.values[0]
                        G.nodes[name]['nparty']= 0 if G.nodes[name]['party'] == 'Democratic Party' else 1

            if not G.has_node(row.mentioned):
                if (row.mentioned in catalogue_df.name.values):
                    name = catalogue_df.loc[catalogue_df['name_low'] == row.mentioned.lower()].name.values[0]
                    if not G.has_node(name):
                        G.add_node(name)
                        G.nodes[name]['party']= catalogue_df.loc[catalogue_df['name_low'] == row.mentioned.lower()].parties.values[0]
                        G.nodes[name]['nparty']= 0 if G.nodes[name]['party'] == 'Democratic Party' else 1

            if (row.mentioner in catalogue_df.name.values and row.mentioned in catalogue_df.name.values):
                G.add_edge(row.mentioner, row.mentioned, weight=row.weight, sentiment=row.sentiment)
        G_all.append(G)
    return(G_all)
...
def get_plotting_info(G_all):
    ...
    for G in G_all:
        ...
        sorted_centr0=[x[0] for x in sorted(centr0.items(), key=lambda x: x[1], reverse=True)[:10]]
        g1=G.subgraph(communities[1])
        centr1=nx.degree_centrality(g1)
        sorted_centr1=[x[0] for x in sorted(centr1.items(), key=lambda x: x[1], reverse=True)[:10]]
        sorted_centers=[]
    ...
    index = [0,edges[0]+1,sum(edges[0:2])+2,sum(edges[0:3])+3,sum(edges[0:4])+4]
    return edges,nodes,index,g_centr_parts_all,Laycentr_all,labels_centr_all,colmap_centr_all
...
def plotly_graph_bar(g,L,labels,colormap,edges,nodes,index):#CF: m_y mention_y dataframe containing index of nodes, edges to use
    ...
    for j in range(len(g)): #CF: embed codes in a loop to interate through years
        edge_index = 0
        node_index = 0
        Xe=[]
        Ye=[]
        ...
        edge_count =  edges[j]#CF: get edge indexes to use from m_y for each year
        node_count =  nodes[j] #CF: get node indexes to use from m_y for each year
        #add edges
        ...
        try:
            scatters.append(go.Scatter(x=Xv[node_index:node_index+node_count],y=Yv[node_index:node_index+node_count],mode='markers',
                                   marker=dict(color=colormap[j][node_index:node_index+node_count],size=14,#[np.log(g[i].degree[v])*200 + 5 for v in g[i]][:node_count],#[np.log(g[i].degree[v])*400 + 5 for v in g[i]]
                                   line=dict(color=colormap[j][node_index:node_index+node_count],width=2)),
                                   hovertemplate=[label+'<extra></extra>' for label in labels[j]]))
        except:
            print(i)
    ...
    for i in range(len(g)):
        ...
        l=dict(label=topic,method="update",args=[{"visible": l_visable},{"title": topic,"annotations":[dict(
            ax=(L[i][edge[0]][0] + L[i][edge[1]][0]) / 2,
            ay=(L[i][edge[0]][1] + L[i][edge[1]][1]) / 2, axref='x', ayref='y',
            x=(L[i][edge[1]][0] * 3 + L[i][edge[0]][0]) / 4,
            y=(L[i][edge[1]][1] * 3 + L[i][edge[0]][1]) / 4, xref='x', yref='y',
            showarrow=True,
            arrowcolor='rgb' + str(matplotlib.colors.to_rgb(matplotlib.cm.coolwarm((np.sign(g[i].get_edge_data(u=edge[0], v=edge[1])[0]['sentiment'])*
                                                                                        np.power(abs(g[i].get_edge_data(u=edge[0], v=edge[1])[0]['sentiment']),0.5)
                                                                                        +1)/2))),
            arrowhead=2,
            arrowsize=np.log(g[i].get_edge_data(u=edge[0], v=edge[1])[0]['weight']),
            arrowwidth=1,
            opacity=1
